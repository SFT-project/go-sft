...
// Package sftstats implements the network stats reporting service.
package sftstats
...
// backend encompasses the bare-minimum functionality needed for sftstats reporting
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(sftstats)
	return nil
}
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the sftstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to sftstats", "latency", latency)
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to sftstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to sftstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to sftstats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to sftstats")
