...
type ChainIndexer struct {
	chainDb  sftdb.Database      // Chain database to index the data from
	indexDb  sftdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb sftdb.Database, indexDb sftdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
