...
import (
	...
	"github.com/sft-project/go-sft/core/state/snapshot"
...
type BlockChain struct {
	...
	db     sftdb.Database // Low level persistent database to store final content in
...
// Processor.
func NewBlockChain(db sftdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db sftdb.KeyValueWriter, header *types.Header) (uint64, bool) {
	...
	delFn := func(db sftdb.KeyValueWriter, hash common.Hash, num uint64) {
	...
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - sftdb.IdealBatchSize)
			}
