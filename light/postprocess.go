...
func GetChtRoot(db sftdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func StoreChtRoot(db sftdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
type ChtIndexerBackend struct {
	disablePruning       bool
	diskdb, trieTable    sftdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db sftdb.Database, odr OdrBackend, size, confirms uint64, disablePruning bool) *core.ChainIndexer {
...
func (c *ChtIndexerBackend) Prune(threshold uint64) error {
	...
	for {
		...
		if batch.ValueSize() > sftdb.IdealBatchSize {
...
func GetBloomTrieRoot(db sftdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db sftdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewBloomTrieIndexer(db sftdb.Database, odr OdrBackend, parentSize, size uint64, disablePruning bool) *core.ChainIndexer {
